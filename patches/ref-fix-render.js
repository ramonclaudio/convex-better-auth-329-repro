...
function useUseAuthFromBetterAuth(authClient, initialToken) {
    ...
    const cachedTokenRef = useRef(initial);
    const [cachedToken, setCachedTokenState] = useState(initial);
    const setCachedToken = (v) => {
        cachedTokenRef.current = v;
        setCachedTokenState(v);
    };
    ...
    return useMemo(() => function useAuthFromBetterAuth() {
        ...
        if (sessionId !== lastSessionIdRef.current) {
            const isRotation = sessionId !== undefined && lastSessionIdRef.current !== undefined;
